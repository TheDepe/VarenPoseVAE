...
parser.add_argument(
    "--dataset_path",
    type=str,
    default="/ps/project/horses_in_motion/VARENset_subset",
)  #'./data/shapenet.hdf5')
...
# Training
parser.add_argument(
    "--epochs", type=int, default=float("inf")
...
model = VarenPoserTrainingExtension(varen_path=args.varen_model_path).to(
    device
)
params_to_optimize = [
    p for name, p in model.named_parameters() if "body_model" not in name
]
...
logger.info(
    f"[Initialisation] Size of model: {sum(p.numel() for p in params_to_optimize)}"
)
...
while epoch <= args.epochs:
    ...
    if epoch % 1 == 0 or epoch == args.epochs:
        ...
        idx = torch.randint(
            0, batchsize, (1,)
        ).item()  # Extract the integer value
        trimesh.Trimesh(
            vertices=orig[idx], faces=model.body_model.faces
        ).export(log_dir / f"input_epoch_{epoch}.ply")
        trimesh.Trimesh(
            vertices=recon[idx], faces=model.body_model.faces
        ).export(log_dir / f"recon_epoch_{epoch}.ply")
        ...
        filtered_state_dict = {
            k: v for k, v in state_dict.items() if "body_model" not in k
        }

        torch.save(
            filtered_state_dict,
            log_dir / f"VAREN_pose_VAE_epoch_{epoch}.pth",
        )
